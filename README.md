...
`default_runfiles` from [DefaultInfo][] and compares their
`file.owner.repo_name` values to `_THIS_REPO`.
...
`*.dist-info/METADATA` files. A quick [print()][] showed that
`DefaultInfo.default_runfiles` includes these files.

The `_collect_pip_names_from_metadata_files()` helper parses the names from
these files paths using `_name_from_metadata_path()`. This is because neither a
[rule][] nor an [aspect][] has any ability to _read_ the contents of a file.

A [repository_rule][], via [repository_ctx.read][], _could_ read these files and
generate `.bzl` files that other rules could depend upon and read. Doing so
would be more robust than trying to parse the `*.dist-info/METADATA` file path.

For the purposes of the original question, this would solve the problem without
patching `rules_python` or navigating the `genquery` JSON output.

```sh
$ bazel build //:{metas,aspect}

INFO: Analyzed 2 targets (72 packages loaded, 3120 targets configured).
INFO: Found 2 targets...
INFO: Elapsed time: 5.252s, Critical Path: 0.01s
INFO: 3 processes: 3 internal.
INFO: Build completed successfully, 3 total actions

$ cat bazel-bin/meta-list.txt

pyfakefs
urllib3
zipp

$ cat bazel-bin/aspect-list.txt
...
[aspect]: https://bazel.build/extending/aspects
...
[print()]: https://bazel.build/rules/lib/globals/all#print
